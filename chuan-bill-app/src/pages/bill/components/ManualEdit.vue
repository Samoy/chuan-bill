...
<template>
  <wd-form :ref="form" :model="fromData" custom-class="flex-1 overflow-auto pb-6">
    <wd-radio-group v-model="fromData.type" shape="button" custom-class="flex">
    ...
    <wd-input
      v-model="fromData.amount" no-border type="digit" placeholder="0.00" custom-class="mt-2"
    ...
    <wd-input
      v-model="fromData.name" type="text" placeholder="账单名称" custom-class="mt-2" no-border
    ...
    <view class="mt-3 flex gap-3">
      <view class="flex-1">
        ...
        <wd-picker v-model="fromData.categoryId" title="请选择账单类目" placeholder="请选择" custom-class="mt-2 " />
      </view>
      <view class="flex-1">
        <view class="text-xs text-gray-500">
          <text class="i-lucide:credit-card mr-2" />支付方式
        </view>
        <wd-picker v-model="fromData.paymentMethodId" title="请选择支付方式" placeholder="请选择" custom-class="mt-2" />
      </view>
    </view>
    <wd-divider color="#dddddd" custom-class="!mt-2 !px-0" />
    ...
    <template v-if="isShared">
      <wd-picker v-model="fromData.familyId" custom-class="mt-3" title="请选择所要共享的家庭" />
    </template>
    <view class="mt-3 text-xs text-gray-500">
      备注
    </view>
    <wd-textarea
      v-model="fromData.remark" placeholder="可输入关于该账单的更多信息" custom-class="mt-2 !p-0" no-border
      custom-input-class="!text-4"
      :maxlength="500"
      show-word-limit
    />
    <wd-button type="primary" block custom-class="!pos-absolute bottom-[10px] left-4 right-4 z-1">
      保存
    </wd-button>
  </wd-form>
</template>
