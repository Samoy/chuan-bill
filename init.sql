...
CREATE TABLE IF NOT EXISTS `t_category`(
    ...
    `user_id` VARCHAR(64) NOT NULL COMMENT '用户ID, 为空表示系统预设类目',
    `create_time` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    `update_time` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
    `deleted` TINYINT(1) DEFAULT 0 COMMENT '是否删除，0未删除，1已删除',
    KEY `idx_type` (`type`),
...
CREATE TABLE IF NOT EXISTS `t_payment_method`(
    ...
    `user_id` VARCHAR(64) NOT NULL COMMENT '用户ID, 为空表示系统预设支付方式',
    `create_time` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    `update_time` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
    `deleted` TINYINT(1) DEFAULT 0 COMMENT '是否删除，0未删除，1已删除',
    KEY `idx_user_id` (`user_id`),
...
-- 支出类目
INSERT INTO `t_category` (`id`, `name`, `icon`, `type`, `sort_order`, `is_default`) VALUES
...
-- 收入类目
INSERT INTO `t_category` (`id`, `name`, `icon`, `type`, `sort_order`, `is_default`) VALUES
('cat_inc_001', '工资', 'icon-salary', 'income', 1, 1),
...
-- 初始化系统预设支付方式
-- ===============================
INSERT INTO `t_payment_method` (`id`, `name`, `icon`, `sort_order`, `is_default`) VALUES
